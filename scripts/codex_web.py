...
import argparse, base64, binascii, json, os, re, subprocess, sys, time
...
<main>
  ...
  <section class="pane"><div class="pane-head"><div class="pane-title">Conversation</div><div id="base" class="hint">No branch base selected. Click a hash to copy it.</div></div><div id="chat"></div><div id="composer"><textarea id="prompt" placeholder="Prompt. Send interrupts/resumes; Fresh starts a new session; Branch starts a sibling worktree from selected commit."></textarea><div id="attachments" class="attachments"></div><div class="row"><label><input class="file-input" id="files" type="file" accept="image/*" multiple onchange="uploadFiles()"> <button type="button" onclick="$('files').click()">Attach screenshot</button></label><button onclick="send('send')">Send / interrupt</button><button onclick="send('fresh')">Fresh</button><button onclick="send('branch')">Branch from selected</button><button onclick="clearBase()">Clear base</button></div></div></section>
...
<script>
let baseCommit='', selectedCommit='', queued=null, repoTimer=null, attachments=[];
...
function renderAttachments(){$('attachments').innerHTML=attachments.map(a=>`<span class="chip">${esc(a.name||a.path)}</span>`).join('')}
async function uploadFiles(){
  for(let f of $('files').files){
    ...
    attachments.push(await api('/api/upload',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({repo:$('repo').value,name:f.name,content_type:f.type||'image/png',data:data})}));
  }
  $('files').value=''; renderAttachments();
}
...
async function loadStatus(){
  ...
  let lines=[];
  if(queued)lines.push(`<button class="state-line queued" onclick="showTranscript('', '${esc(queued.log||'')}')">Queued: ${esc(queued.mode)} PID ${esc(String(queued.pid||''))} · Full transcript</button>`);
...
async function send(mode){
  ...
  queued={mode:mode,prompt:p,pid:j.process&&j.process.pid,log:j.process&&j.process.log}; if(j.worktree){$('repo').value=j.worktree.path; clearBase()}
...
window.onload=async()=>{let c=await api('/api/config'); $('repo').value=c.repo; $('repo').addEventListener('input',repoPathChanged); $('repo').addEventListener('change',repoPathChanged); $('repo').addEventListener('keydown',e=>{if(e.key==='Enter')repoPathChanged()}); await refreshAll()}
</script>'''
