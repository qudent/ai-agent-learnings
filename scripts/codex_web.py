...
<main>
  ...
  <section class="pane"><div class="pane-head"><div class="pane-title">Conversation</div><div id="base" class="hint">No branch base selected. Click a hash to copy it.</div></div><div id="chat"></div><div id="composer"><textarea id="prompt" placeholder="Prompt. Send queues behind an active run; Fresh starts a new session; Branch starts a child worktree from selected commit."></textarea><div id="attachments" class="attachments"></div><div class="row"><label><input class="file-input" id="files" type="file" accept="image/*" multiple onchange="uploadFiles()"> <button type="button" onclick="$('files').click()">Attach screenshot</button></label><button onclick="send('send')">Send / queue</button><button onclick="send('fresh')">Fresh</button><button onclick="send('branch')">Branch from selected</button><button onclick="clearBase()">Clear base</button></div></div></section>
