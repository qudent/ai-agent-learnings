...
async function uploadFileList(files){
  for(let f of Array.from(files||[]).filter(f=>(f.type||'').startsWith('image/'))){
...
async function handlePaste(e){
  ...
  if(files.some(f=>(f.type||'').startsWith('image/'))){e.preventDefault(); await uploadFileList(files)}
}
