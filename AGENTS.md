...
Dispatcher agents are active orchestration threads, not ceremony-only trampolines. They reconcile the Agent Context Pack, inspect active runs/interruption intent, update task routing surfaces (`STATUS.md`, optional `jj` task mirrors, and `agents/<slug>/inbox.md` when following up with an existing agent), then do at least one meaningful routing/work slice themselves before stopping. Plain `codex_commit` is for a single narrow implementation agent that was already scoped by a human or dispatcher; broad or recursive work should go through `codex_dispatch`, which generates an Agent Context Pack from branch-local `STATUS.md`, active transcript pointers, relevant agent profiles/inboxes, transcript tails, live wrapper state, optional JJ task surface, and the audit trail. Child agents launched with `codex_spawn` receive bounded fresh context, inherit ancestry through `CODEX_WRAP_CALLED_BY`, and must keep instructions/audit in files rather than long commit messages.
